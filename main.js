...
__export(main_exports, {
  default: () => DailyRemindersPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian = require("obsidian");
var import_lunar_javascript = __toESM(require_lunar_javascript());
var DEFAULT_SETTINGS = {
  reminderDocPath: "antitest/\u5907\u5FD8\u65E5\u671F.md",
  dailyNoteFolder: "journals",
  triggerTime: "06:00",
  lastCheckDate: "",
  insertHeader: "## \u{1F4C5} \u4ECA\u65E5\u63D0\u9192"
};
var DailyRemindersPlugin = class extends import_obsidian.Plugin {
  async onload() {
    await this.loadSettings();
    this.addSettingTab(new DailyRemindersSettingTab(this.app, this));
  ...
  checkTimeTrigger() {
    const now = moment();
    const trigger = moment(this.settings.triggerTime, "HH:mm");
    const todayStr = now.format("YYYY-MM-DD");
    if (this.settings.lastCheckDate === todayStr)
      return;
    if (now.hour() === trigger.hour() && now.minute() === trigger.minute()) {
      this.checkReminders();
    }
  }
  async checkReminders(force = false) {
    const todayStr = moment().format("YYYY-MM-DD");
    if (!force && this.settings.lastCheckDate === todayStr) {
      return;
    }
    const file = this.app.vault.getAbstractFileByPath(this.settings.reminderDocPath);
    if (!(file instanceof import_obsidian.TFile)) {
      if (force)
        new import_obsidian.Notice(`\u672A\u627E\u5230\u5907\u5FD8\u6587\u4EF6: ${this.settings.reminderDocPath}`);
      return;
    }
    const content = await this.app.vault.read(file);
    const matches = this.parseReminders(content);
    if (matches.length > 0) {
      await this.insertIntoDailyNote(matches);
      if (force)
        new import_obsidian.Notice(`\u5DF2\u63D2\u5165 ${matches.length} \u6761\u63D0\u9192\u3002`);
    } else {
      await this.insertIntoDailyNote([]);
      if (force)
        new import_obsidian.Notice("\u4ECA\u5929\u6CA1\u6709\u63D0\u9192\u4E8B\u9879\u3002");
    }
    if (!force) {
      this.settings.lastCheckDate = todayStr;
  ...
  parseReminders(content) {
    ...
    for (let line of lines) {
      ...
      if (parts.length < 4)
        continue;
      const [eventName, type, datePattern, ruleStr] = parts;
      const daysDiff = this.calculateDaysDiff(type, datePattern, today);
      if (daysDiff === null)
        continue;
      const ruleMatched = this.checkRuleMatch(ruleStr, daysDiff);
      if (ruleMatched) {
        let timeDesc = daysDiff === 0 ? "\u4ECA\u5929" : `${daysDiff}\u5929\u540E`;
        results.push(`${eventName} (${timeDesc})`);
      }
    }
    return results;
  }
  // Returns number of days until the NEXT occurrence (0 if today)
  // Returns null if parsing fail
  calculateDaysDiff(type, pattern, today) {
    try {
      if (type === "\u516C\u5386") {
        return this.getNextSolarDiff(pattern, today);
      } else if (type === "\u519C\u5386") {
        return this.getNextLunarDiff(pattern, today);
      }
    } catch (e) {
      console.error(e);
      return null;
  ...
  getNextSolarDiff(pattern, today) {
    const currentYear = today.year();
    const currentMonth = today.month() + 1;
    const currentDay = today.date();
    let targetDate = null;
    const monthlyMatch = pattern.match(/每月(\d+)日?/);
    if (monthlyMatch) {
      const day = parseInt(monthlyMatch[1]);
      if (currentDay <= day) {
        targetDate = moment(`${currentYear}-${currentMonth}-${day}`, "YYYY-MM-DD");
      } else {
        targetDate = moment(`${currentYear}-${currentMonth}-${day}`, "YYYY-MM-DD").add(1, "month");
      }
    } else if (pattern.match(/每年/)) {
      const m = pattern.match(/(\d+)月(\d+)日/);
      if (m) {
        const month = parseInt(m[1]);
        const day = parseInt(m[2]);
        targetDate = moment(`${currentYear}-${month}-${day}`, "YYYY-MM-DD");
        if (targetDate.isBefore(today)) {
          targetDate.add(1, "year");
        }
      }
    } else {
      const manual = moment(pattern, ["YYYY-MM-DD", "YYYY/MM/DD"], true);
      if (manual.isValid()) {
        if (manual.isSameOrAfter(today)) {
          targetDate = manual;
        } else {
          return null;
        }
  ...
  getNextLunarDiff(pattern, today) {
    const { lMonth, lDay } = this.parseLunarString(pattern);
    if (!lMonth || !lDay)
      return null;
    const currentYear = today.year();
    for (let y = currentYear; y <= currentYear + 1; y++) {
      try {
        const lunar = import_lunar_javascript.Lunar.fromYmd(y, lMonth, lDay);
        const solar = lunar.getSolar();
        const sDate = moment(`${solar.getYear()}-${solar.getMonth()}-${solar.getDay()}`, "YYYY-MM-DD");
        if (sDate.isSameOrAfter(today)) {
          return sDate.diff(today, "days");
        }
      } catch (e) {
        continue;
      }
  ...
  parseLunarString(str) {
    ...
    if (parts.length === 2) {
      let mStr = parts[0].trim();
      let dStr = parts[1].trim();
      if (monthMap[mStr])
        month = monthMap[mStr];
      if (dStr.length >= 1) {
        let prefix = dStr[0];
        let suffix = dStr.substring(1);
        if (dayPrefixMap[prefix] !== void 0) {
          day += dayPrefixMap[prefix];
          if (suffix && digitMap[suffix]) {
            day += digitMap[suffix];
          } else if (prefix === "\u5341" && !suffix) {
          }
          if ((prefix === "\u4E8C\u5341" || prefix === "\u4E09\u5341" || prefix === "\u5EFF" || prefix === "\u5345") && !suffix) {
          }
        } else if (dStr === "\u5341") {
          day = 10;
        } else if (digitMap[dStr]) {
          day = digitMap[dStr];
        }
        if (day === 0) {
          if (dStr === "\u5341\u4E94")
    ...
    return { lMonth: month, lDay: day };
  }
  checkRuleMatch(rule, diff) {
    const subRules = rule.split("+").map((s) => s.trim());
    for (const r of subRules) {
      if (r.includes("\u5F53\u65E5")) {
        if (diff === 0)
          return true;
      } else if (r.match(/前(\d+)天/)) {
        const m = r.match(/前(\d+)天/);
        if (m) {
  ...
  async insertIntoDailyNote(reminders) {
    const todayStr = moment().format("YYYY-MM-DD");
    let file = null;
    const allFiles = this.app.vault.getFiles();
    file = allFiles.find((f) => f.name === `${todayStr}.md`) || null;
    if (!file) {
      let folderPath = this.settings.dailyNoteFolder;
      if (!folderPath)
        folderPath = "/";
      if (folderPath !== "/" && !this.app.vault.getAbstractFileByPath(folderPath)) {
        await this.app.vault.createFolder(folderPath);
      }
      const path = (0, import_obsidian.normalizePath)(`${folderPath}/${todayStr}.md`);
      file = await this.app.vault.create(path, "");
    }
    const text = await this.app.vault.read(file);
    const header = this.settings.insertHeader;
    let finalContent = text;
    const hasHeader = text.includes(header);
    if (reminders.length === 0) {
      const noReminderMsg = "\u4ECA\u65E5\u65E0\u91CD\u8981\u4E8B\u9879\u9700\u8981\u63D0\u9192\u3002";
      if (text.includes(noReminderMsg))
        return;
      if (!hasHeader) {
        finalContent += `
...
${noReminderMsg}`;
    ...
    const newReminders = reminders.filter((r) => !text.includes(r.split(" ")[0]));
    if (newReminders.length === 0)
      return;
    const block = newReminders.map((r) => `- [ ] ${r}`).join("\n");
    if (!hasHeader) {
      finalContent += `
...
var DailyRemindersSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Daily Reminders Settings" });
    new import_obsidian.Setting(containerEl).setName("\u5907\u5FD8\u5F55\u6587\u4EF6\u8DEF\u5F84").setDesc("\u76F8\u5BF9\u4E8E\u5E93\u6839\u76EE\u5F55\u7684\u8DEF\u5F84\uFF0C\u4F8B\u5982: antitest/\u5907\u5FD8\u65E5\u671F.md").addText((text) => text.setPlaceholder("path/to/doc.md").setValue(this.plugin.settings.reminderDocPath).onChange(async (value) => {
      this.plugin.settings.reminderDocPath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("\u65E5\u8BB0\u6587\u4EF6\u5939").setDesc("\u65B0\u5EFA\u65E5\u8BB0\u65F6\u7684\u9ED8\u8BA4\u6587\u4EF6\u5939").addText((text) => text.setValue(this.plugin.settings.dailyNoteFolder).onChange(async (value) => {
      this.plugin.settings.dailyNoteFolder = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("\u89E6\u53D1\u65F6\u95F4").setDesc("\u6BCF\u5929\u81EA\u52A8\u68C0\u67E5\u7684\u65F6\u95F4 (HH:mm)").addText((text) => text.setValue(this.plugin.settings.triggerTime).onChange(async (value) => {
      this.plugin.settings.triggerTime = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("\u63D2\u5165\u6807\u9898").setDesc("\u63D0\u9192\u4E8B\u9879\u5217\u8868\u7684\u6807\u9898").addText((text) => text.setValue(this.plugin.settings.insertHeader).onChange(async (value) => {
      this.plugin.settings.insertHeader = value;
      await this.plugin.saveSettings();
    }));
  }
};
